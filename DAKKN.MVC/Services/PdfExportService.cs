...
namespace DAKKN.Appearence.Services
{
    public interface IPdfExportService
    {
        ...
        byte[] GenerateUndeliveredCustomOrdersPdf(List<ExportUndeliveredCustomOrderDto> orders);
    }
    ...
    public class PdfExportService : IPdfExportService
    {
        ...
        public byte[] GenerateUndeliveredCustomOrdersPdf(List<ExportUndeliveredCustomOrderDto> orders)
        {
            QuestPDF.Settings.License = LicenseType.Community;
            ...
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Element(c => ComposeCustomOrderHeader(c, isArabic));
                    page.Content().Element(c => ComposeCustomOrderContent(c, orders, isArabic));
                    page.Footer().Element(c => ComposeCustomOrderFooter(c, isArabic));
                });
            }).GeneratePdf();
        }
        ...
        private void ComposeCustomOrderHeader(IContainer container, bool isArabic)
        {
            container.Column(column =>
            {
                column.Item().Row(row =>
                {
                    if (isArabic)
                    {
                        ...
                        {
                            col.Item().Text("DAKKN").FontSize(22).Bold().FontColor(Colors.Blue.Darken3).AlignRight();
                            col.Item().Text(_localizer["pdf_undelivered_title_custom"]).FontSize(14).FontColor(Colors.Grey.Darken2).AlignRight();
                        });
                    }
                    else
                    {
                        ...
                        {
                            col.Item().Text("DAKKN").FontSize(22).Bold().FontColor(Colors.Blue.Darken3).AlignLeft();
                            col.Item().Text(_localizer["pdf_undelivered_title_custom"]).FontSize(14).FontColor(Colors.Grey.Darken2).AlignLeft();
                        });
        ...
        private void ComposeCustomOrderContent(IContainer container, List<ExportUndeliveredCustomOrderDto> orders, bool isArabic)
        {
            container.Table(table =>
            {
                table.ColumnsDefinition(columns =>
                ...
                table.Header(header =>
                {
                    HeaderCellStyle(header.Cell(), isArabic).Text(_localizer["pdf_number"]).Bold().FontColor(Colors.White);
                    HeaderCellStyle(header.Cell(), isArabic).Text(_localizer["pdf_customer_name"]).Bold().FontColor(Colors.White);
                    HeaderCellStyle(header.Cell(), isArabic).Text(_localizer["pdf_address"]).Bold().FontColor(Colors.White);
                    HeaderCellStyle(header.Cell(), isArabic).Text(_localizer["pdf_phone"]).Bold().FontColor(Colors.White);
                });
                ...
                foreach (var order in orders)
                {
                    ...
                    DataCellStyle(table.Cell(), bgColor, isArabic).Text(index.ToString());
                    DataCellStyle(table.Cell(), bgColor, isArabic).Text(order.CustomerName);
                    DataCellStyle(table.Cell(), bgColor, isArabic).Text(order.ShippingAddress);
                    DataCellStyle(table.Cell(), bgColor, isArabic).Text(order.CustomerPhone);
        ...
        private void ComposeCustomOrderFooter(IContainer container, bool isArabic)
        {
            container.Row(row =>
            {
                row.RelativeItem().AlignLeft().Text(_localizer["pdf_footer_custom"]).FontSize(8).FontColor(Colors.Grey.Medium);
